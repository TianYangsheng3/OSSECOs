...
class Lstm(nn.Module):
    ...
    def forward(self, x):
        ...
        health = F.relu(self.fc1(out.view(-1, self.hidden_dims)))
        health = F.sigmoid(self.fc2(health))
...
def train(traindata, lstm, criterion, optimizer, batch_size, seq_num, epoch): ### data 50*320*23
    ...
    for i in range(epoch):
        for batch_i in range(int(traindata.size(1)/batch_size)):
            ...
            target_1 = traindata[1:seq_num+1, start:end, 1:]
            target_2 = traindata[1:seq_num+1, start:end, 0:1]
            target_2 = target_2.reshape(-1,1)

            optimizer.zero_grad()

            outputs_1, outputs_2 = lstm(inputs)
            ...
            if batch_i == (int(traindata.size(1)/batch_size)-1) and i==epoch-1:
                cmp(i, target_1, outputs_1, 'feature')
                cmp(i, target_2.reshape(-1, batch_size, 1), outputs_2.reshape(-1, batch_size, 1), 'health')
...
def test(testdata, lstm, criterion, optimizer, batch_size, seq_num):
    ...
    with torch.no_grad():
        for batch_i  in range(int(testdata.size(1)/batch_size)):
            ...
            target_1 = testdata[1:seq_num+1, start:end, 1:]
            target_2 = testdata[1:seq_num+1, start:end, 0:1]
            target_2 = target_2.reshape(-1,1)

            predictions_1, predictions_2 = lstm(inputs)
            loss = criterion(predictions_1, target_1) + criterion(predictions_2, target_2)
            test_loss += loss.item()
            print('testdata: [%5d] loss: %.3f ' % (batch_i+1, test_loss))
            if batch_i == (int(testdata.size(1)/batch_size)-1) :
                cmp(0, target_1, predictions_1, "feature")
                cmp(0, target_2.reshape(-1, batch_size, 1), predictions_2.reshape(-1, batch_size, 1), "health")
    
    print("Finishing testing!")


def cmp(epoch, groud, pred, ps):
    ...
    for batch_i in range(batch_size):
        ...
        for features_i in range(features):
            ...
            plt.legend()
            plt.title(ps+": epoch " + str(epoch) + " batch " + str(batch_i) + " feature : " + str(features_i))
            filepath = "fig//"+ps+"_epoch_"+str(epoch) + "_batch_" + str(batch_i) + "_feature_" + str(features_i)+".pdf"
...
if __name__ == '__main__':
    ...
    seq_num = 50
    epoch = 500
    input_dims, hidden_dims, layers, batch_size = 22, 22, 2, 8
    lstm = Lstm(input_dims, hidden_dims, layers, batch_size)
    print(lstm)
    criterion = nn.MSELoss()
    optimizer = optim.SGD(lstm.parameters(), lr=0.1, momentum=0.9)

    # print("before training: ", lstm.fc1.bias.grad)
    train(traindata, lstm, criterion, optimizer, batch_size, seq_num, epoch)
    # print("after training: ", lstm.fc1.bias.grad)
